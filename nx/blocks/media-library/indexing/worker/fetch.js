...
export async function workerDaFetch(url, imsToken, opts = {}) {
  ...
  if (imsToken) {
    ...
    // For admin.ent-aem.page URLs, add x-content-source-authorization header
    // This is required for writes and API access (matches daFetch.js:27-29)
    if (url.startsWith('https://admin.ent-aem.page')) {
 ...
 * @param {string} daOrigin - DA origin (e.g., 'https://admin.ent-da.live')
 * @param {string} imsToken - IMS access token
...
/**
 * Fetch paginated data from admin.ent-aem.page endpoints
...
function buildAemPageMarkdownUrl(pagePath, org, repo, ref = 'main') {
  ...
  return `https://${ref}--${repo}--${org}.ent-aem.page${path}`;
}
...
/**
 * Fetch markdown from .ent-aem.page using CORS proxy
