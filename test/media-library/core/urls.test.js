...
describe('urls', () => {
  describe('isInternalToSite', () => {
    ...
    it('returns true for same org/repo .ent-aem.page URL', () => {
      expect(isInternalToSite('https://main--blog--adobe.ent-aem.page/docs', org, repo)).to.be.true;
    });

    it('returns true for same org/repo .ent-aem.live URL', () => {
      expect(isInternalToSite('https://main--blog--adobe.ent-aem.live/media/image.png', org, repo)).to.be.true;
    });
    ...
    it('returns false for different repo on .ent-aem.page', () => {
      expect(isInternalToSite('https://main--other--adobe.ent-aem.page/docs', org, repo)).to.be.false;
    });

    it('returns false for different org on .ent-aem.page', () => {
      expect(isInternalToSite('https://main--blog--other.ent-aem.page/docs', org, repo)).to.be.false;
    });
  ...
  describe('getDedupeKey', () => {
    it('returns media hash basename for hashed media URL', () => {
      const url = 'https://main--blog--adobe.ent-aem.live/media/media_123abc456def.png';
      const key = getDedupeKey(url);
      expect(key).to.equal('media_123abc456def.png');
    });

    it('returns media hash basename with query params', () => {
      const url = 'https://main--blog--adobe.ent-aem.live/media/media_abc123.png?width=2000';
      const key = getDedupeKey(url);
      expect(key).to.equal('media_abc123.png');
    });

    it('returns pathname for non-hashed internal path', () => {
      const url = 'https://main--blog--adobe.ent-aem.live/docs/image.png';
      const key = getDedupeKey(url);
  ...
  describe('isDeliveryStandaloneUrl', () => {
    ...
    it('returns true for matching .ent-aem.page host', () => {
      expect(isDeliveryStandaloneUrl(
        'https://main--blog--adobe.ent-aem.page/media/media_abc.png',
        org,
        repo,
      )).to.be.true;
    });

    it('returns true for matching .ent-aem.live host', () => {
      expect(isDeliveryStandaloneUrl(
        'https://main--blog--adobe.ent-aem.live/videos/clip.mp4',
        org,
        repo,
      )).to.be.true;
    });
    ...
    it('classify from raw only: canonicalize maps www.adobe.com onto delivery host', () => {
      ...
      expect(canon).to.include('ent-aem.page');
    ...
    it('returns false when org or repo missing', () => {
      expect(isDeliveryStandaloneUrl(
        'https://main--blog--adobe.ent-aem.page/media/x.png',
        '',
        repo,
      )).to.be.false;
    });
  ...
  describe('folderPathFromAssetUrl', () => {
    it('returns parent folder of pathname', () => {
      const folder = folderPathFromAssetUrl(
        'https://main--blog--adobe.ent-aem.page/media/nested/file.png',
        'adobe',
        'blog',
      );
    ...
    it('returns empty string for root file', () => {
      expect(folderPathFromAssetUrl(
        'https://main--blog--adobe.ent-aem.page/top.png',
        'adobe',
        'blog',
      )).to.equal('');
    });
